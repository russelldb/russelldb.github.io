...
that can be mitigated against with a simple config change in Riak
...
and Handoff in Riak as all these processes have a part to play.

As the quote above suggest, it is important to point out that the
...
As a reminder: an actor in a version vector must increment its counter
each time it updates a value. If an actor can not read its own latest
update in the version vector, it cannot issue a new event that is
...
allows concurrent writes to the same value, and it so allows a value
...
concurrently written value become siblings. If there is no concurrent
update only the tombstone is recorded on disk. A request to read a key
that only has a tombstone value will return `not_found`. Riak
understands tombstones to mean logical deletion. A
...
When receiving a read request Riak asks every replica for the
value. Each replica responds with it's local copy. Using the Version
Vector Riak can detect if some replica is behind, or in conflict, and
send the most up-to-date value to that replica. The replica then
stores the correct value. It is a curious fact that _all_ the replying
...
between keys and resolves the differences by asking Riak to perform a
`GET` operation internally, thus triggering read repair. AAE is not
strictly necessary for the instance of the bug we are discussing. In
the example AAE and Read Repair are interchangable.
...
The example I'm going to use has deletes, read repair, AAE, and hand
...
tombstone as a _fallback_. The client is notified the delete succeed,
...
Later, a AAE or a read repair will spread the tombstone to nodes `A`
and `B`.
...
reclaimation issue remains. If you are at all concerned, running Riak
prior to 2.1, and delete and recreate keys, then set `delete_mode =
keep` in your config.

## 4. Fault Tolerant?

This example is long and convuluted, thanks for staying with me. I
chose this example as it doesn't involve any extravagent failure,
...
Worry not: the main likely cause of this issue, doomstones, can be
addressed with the simple config change to `delete_mode =
keep`. Doomstones are in no way unique to Riak: databases that use
temporal time and Last Write Wins are far more susceptible.
